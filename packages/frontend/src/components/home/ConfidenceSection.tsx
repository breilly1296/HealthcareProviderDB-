        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {confidenceLevels.map((level) => {
              ...
              return (
                <div key={level.level} className={`card border-l-4 ${styles.border}`}>
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`w-12 h-12 ${styles.bg} rounded-full flex items-center justify-center`}>
                      {level.icon}
                    </div>
                    <div>
                      <div className="font-semibold text-gray-900 dark:text-white">{level.level}</div>
              ...
              {scoringFactors.map((factor) => (
                <div key={factor.title}>
                  ...
                  <p className="text-gray-600 dark:text-gray-300 text-sm">
                    <strong className="dark:text-gray-200">{factor.description.split('—')[0]}</strong>
